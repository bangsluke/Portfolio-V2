...
export async function fetchRoles(): Promise<Role[]> {
	try {
		...
		const response =
			await graphqlClient.request<GetRolesResponse>(GET_ROLES_QUERY);
		...
		console.error(
			'fetchRoles: Error instanceof Error:',
		...
		if (error instanceof Error) {
			...
			if (error.message.includes('fetch')) {
				...
				throw new Error(
					'Cannot connect to backend server. Please ensure the backend is running on http://localhost:5001'
				);
			}
