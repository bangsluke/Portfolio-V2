...
- These `Markdown` notes are processed within the [`sync.js`](./scripts/sync.js) file in the function ([`processMarkdownFile()`](./scripts/sync.js#L309)) to format the `Markdown` ready for display on the site;
  ...
  - In the `Markdown` note content (ignoring the frontmatter) - see ([`processObsidianLinksInContentOnly()`](./scripts/sync.js#L645));
    ...
    - Extracts blocks of text between section headers (which sections are based on the note type - defined in the [`repoConfig.js`](./scripts/repoConfig.js) file) and adds them as properties to the note type for use throughout the site ([`extractSectionsToFrontmatter()`](./scripts/sync.js#L114))
