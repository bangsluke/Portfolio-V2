...
function handleGraphQLError(error: any, operation: string): never {
  ...
  if (error instanceof Error) {
    ...
    if (
      ...
      throw new Error(
        `Network error: Cannot connect to backend server. Please ensure the backend is running.`
      );
    }
...
export async function fetchCompanies(
  ...
  try {
    console.log(
      'fetchCompanies: Starting GraphQL request with options:',
      options
    );
    ...
    const response = await graphqlRequest<GetCompaniesResponse>(
      GET_COMPANIES_QUERY,
      options
    );
    ...
    console.log(
      'fetchCompanies: Number of companies:',
...
export async function fetchCompanyById(
  ...
  try {
    ...
    const response = await graphqlRequest<GetCompanyByIdResponse>(
...
export async function fetchClients(
  ...
  try {
    console.log(
      'fetchClients: Starting GraphQL request with options:',
      options
    );

    // Use enhanced request function with retry logic
    const response = await graphqlRequest<GetClientsResponse>(
      GET_CLIENTS_QUERY,
      options
    );
    ...
    console.log(
      'fetchClients: Number of clients:',
...
export function getActiveCompanies(companies: Company[]): Company[] {
  return companies.filter(
    company => !company.dateEnd || new Date(company.dateEnd) > new Date()
  );
}
...
export function sortCompaniesByDate(
  companies: Company[],
...
export async function fetchCompaniesBatch(
  ...
  try {
    console.log(
      'fetchCompaniesBatch: Fetching companies for nodeIds:',
      nodeIds
    );
...
export async function testGraphQLConnection(): Promise<{
  ...
  try {
    ...
    console.log(
      'testGraphQLConnection: Success - received response:',
      response
    );
